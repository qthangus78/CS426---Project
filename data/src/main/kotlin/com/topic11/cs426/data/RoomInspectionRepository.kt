...
class RoomInspectionRepository(
    ...
    override fun observeInspection(inspectionId: InspectionId): Flow<InspectionSession?> {
        return combine(
            inspectionDao.observeInspection(inspectionId.value),
            inspectionDao.observeAssetName(inspectionId.value),
            inspectionDao.observeAnswers(inspectionId.value),
            inspectionDao.observeEvidence(inspectionId.value),
        ) { inspection, assetName, answers, evidence ->
            if (inspection == null) {
                null
            } else {
                checkNotNull(assetName) {
                    "Inspection asset no longer exists: ${inspectionId.value}"
                }
                com.topic11.cs426.core.database.dao.InspectionDraftRecord(
    ...
    override suspend fun getInspection(inspectionId: InspectionId): InspectionSession? {
        return inspectionDao.getDraft(inspectionId.value)?.toDomain()
    }
    ...
    override suspend fun createInspection(
        ...
        val asset = requireNotNull(database.catalogDao().getAsset(assetId)) {
            "Asset does not exist: $assetId"
        }
        require(asset.name == assetName) {
            "Asset name does not match persisted asset $assetId"
        }
        val template = requireNotNull(database.catalogDao().getTemplateAggregate(templateId)) {
    ...
    private suspend fun answerTypesFor(templateRevisionId: String): Map<String, String> {
        val template = requireNotNull(
            database.catalogDao().getTemplateAggregate(templateRevisionId),
        ) {
